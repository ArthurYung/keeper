...
test("lazy parse properties", ({ expect }) => {
  const extendSource = createKeeper(
    `
sourcekey1 bool
sourcekey2 int
...
key2 *extend
`,
    { extends: { extend: extendSource }, lazy: true },
  );
...
`,
  ...
  expect(keeper.properties).toMatchInlineSnapshot(`Map {}`);
  expect(keeper.from({})).toMatchInlineSnapshot(`
    {
      "key1": "",
      "key2": {
  ...
  expect(keeper.properties).toMatchInlineSnapshot(`
    Map {
      "key1" => {
        "isArray": false,
        "isExtend": false,
